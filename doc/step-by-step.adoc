...
** commit 0da5e06937fb440a09faed60adc0d3fe7ee60561  users fully working, incl new item
...
- One approach is to use the @JsonView methods, which annotate the
  attributes in the models and the methods on the controllers to
  specify which attributes should be shown on this view.  
- Alternatively, we can use @JsonManagedReference, @JsonBackReference
  but that does not allow us to specify which fields we want to show.
- We went with annotating all properties in User and Item with either
  JsonViews.ItemList of JsonViews.UserDetails, and a special abstract
  class was set up to allow for these two views.
...
- http://www.baeldung.com/jackson-serialize-enums   on how to
  ...
  @JsonValue to them. 
- really good overview of how angular works, in only 2 pages:
  http://tutorials.jenkov.com/angularjs/watch-digest-apply.html
- post with lots of pointers, and good separation of what you will
  need to know now vs. tomorrow.
  http://ruoyusun.com/2013/05/25/things-i-wish-i-were-told-about-angular-js.html

** commit 92c2fe8a3225f23276954830cf489ad280d608ea delete button

Make the delete button work  `/resourece/delete/{itemid}/`

- The backend function was easy, a simple delete on the jpa.  Turns
  out we can delete by using the full item object, or by using the
  id.  
- The front end needs to reflect the deleted item.  Ideally, the back
  end would post an update, but we don't have that functionality yet
  so we manually delete the item from our listofitems, using the js
  `splice` function: `splice(i,1)` removes one item at index `i`.
  We iterate backwards through listofitems, as the ending condition
  for forward iteration would have to be changed as we go and
  apparently, js will precompute this.


** commit 3deb2b40035dbc5d48ac093eef4de173b6ca9388  cancel button on edit

This one required a little thought: To cancel an edit on a normal item
is relatively easy because we have our edit fields (titleField, contentField)
separate from our content field (title, content).  However, when a new
item is just created, a cancel should remove that item.  For this, we
created a new variable 'brandnew' (should probably be named
'deleteOnCancel' or so).  

There is a small cornercase bug in this version:  If you save an edit on a new
item, then edit it again and cancel that second edit, the item  will
be deleted.  

** commit 3e5b864a249dfb7f819456bf1ea3ba87b01f1c50  angular update

Angular uses the `$http` function to call back to the server.  In
earlier versions, these call were chained to `.success` and `.error`
to run a function on completion of the request.  

In newer version, this is deprecated and `.then(successfun, errorfun)`
should be used, see
https://code.angularjs.org/1.4.6/docs/api/ng/service/$http. 
Note however that the successfun is called with a `success` object
instead of the data itself.  The data can be found under
...
- polling the server for changes etc.     for a start, see polling
  example: http://jsfiddle.net/rishabh/Kuewm/
- The $http legacy promise methods success and error have been
  deprecated. Use the standard then method
  instead. https://docs.angularjs.org/api/ng/service/$http
- then(successCallback, errorCallback, notifyCallback) – regardless of
  ...
  of the success or error callbacks asynchronously as soon as the
